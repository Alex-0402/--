#!/bin/bash
# 7 GPU DDP 训练启动脚本（使用 GPU 1-7，跳过 GPU 0）
...
# 启动命令 (nproc_per_node=7，使用物理 GPU 1-7)
echo "启动 7 GPU DDP 训练..."
echo "使用 GPU: 1,2,3,4,5,6,7 (物理编号)"
echo "PyTorch 逻辑编号: 0,1,2,3,4,5,6 (通过 --gpu_ids 映射)"
echo ""

torchrun --nproc_per_node=7 --master_port=29505 train.py \
    --pdb_path data/cache \
