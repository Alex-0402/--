...
def main():
    ...
    def decode_with_strategy(strategy_name: str, node_embeddings: torch.Tensor):
        ...
        active_candidates = []
        if fragment_residue_idx is not None and frag_levels is not None:
            fr_idx_cpu = fragment_residue_idx.cpu().numpy().flatten() if hasattr(fragment_residue_idx, 'cpu') else fragment_residue_idx.flatten()
            fl_cpu = frag_levels.cpu().numpy().flatten() if hasattr(frag_levels, 'cpu') else frag_levels.flatten()
            num_residues = int(fr_idx_cpu[-1]) + 1
            res_frag_lengths = [0]*num_residues
            for r_idx in fr_idx_cpu:
                res_frag_lengths[int(r_idx)] += 1
        ...
        if fragment_residue_idx is not None and frag_levels is not None:
            if hasattr(fragment_residue_idx, 'cpu'):
                fr_idx_cpu = fragment_residue_idx.cpu().numpy().flatten()
                fl_cpu = frag_levels.cpu().numpy().flatten()
            else:
                fr_idx_cpu = fragment_residue_idx.flatten()
                fl_cpu = frag_levels.flatten()
        ...
        for k in range(args.num_iterations):
            ...
            if active_candidates:
                ...
                masked_indices_temp = torch.where(masked_positions)[0].cpu().numpy()
                
                for m_idx in masked_indices_temp:
                    r_idx = int(fr_idx_cpu[m_idx])
                    level = int(fl_cpu[m_idx])
                    allowed_tokens = set()
                    
                    for aa in active_candidates[r_idx]:
                        if level < len(res_map[aa]):
                            frag_name = res_map[aa][level]
                            if frag_name in vocab.token_to_idx:
                                allowed_tokens.add(vocab.token_to_idx[frag_name])
                    
                    if allowed_tokens:
                        all_tokens_set = set(range(len(vocab)))
            ...
            if len(active_candidates) > 0:
                for c_idx in commit_indices:
                    c_idx_int = int(c_idx.item())
                    r_idx = int(fr_idx_cpu[c_idx_int])
                    level = int(fl_cpu[c_idx_int])
                    pred_token_idx = int(target_fragments[0, c_idx_int].item())
                    pred_frag_name = vocab.idx_to_token.get(pred_token_idx, "")
                    
                    new_cands = set()
                    for aa in active_candidates[r_idx]:
                        if level < len(res_map[aa]) and res_map[aa][level] == pred_frag_name:
                            new_cands.add(aa)
                    active_candidates[r_idx] = new_cands
